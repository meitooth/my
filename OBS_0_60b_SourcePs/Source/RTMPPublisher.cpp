...
void RTMPPublisher::SendLoop()
{
	...
	while(WaitForSingleObject(hSendSempahore, INFINITE) == WAIT_OBJECT_0)
	{
		...
		{
			OSEnterMutex(hDataMutex);
			int nPacketsSize = queuedPackets.Num();
			if(nPacketsSize  == 0)
			{
				OSLeaveMutex(hDataMutex); 
				break;
			}
			List<NetworkPacket> packetDataList;
			packetDataList.CopyList(queuedPackets);
			queuedPackets.Clear();
			OSLeaveMutex(hDataMutex);
			while(packetDataList.Num() > 0)
			{
				List<BYTE> packetData;
				PacketType type       = packetDataList[0].type;
				DWORD      timestamp  = packetDataList[0].timestamp;
				packetData.TransferFrom(packetDataList[0].data);
				...
				//SendRtpPsToServer(uint8_t* vidoebuf,uint32_t bufsize,__int64 tic,bool bkey)
// 				if (type != PacketType_Audio)
// 				{
// 					char spspps[2048];
// 					int spslen = 0;
// 					App->GetVideoEncoder()->Get_SpsAndPps(spspps,spslen);
// 					SendRtpPsToServer(spspps,spslen, (uint8_t*)packet.m_body,packet.m_nBodySize,timestamp);
// 				}
