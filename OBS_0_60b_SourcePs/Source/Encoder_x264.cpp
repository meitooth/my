...
public:
    X264Encoder(int fps, int width, int height, int quality, CTSTR preset, bool bUse444, ColorDescription &colorDesc, int maxBitrate, int bufferSize, bool bUseCFR)
    {
		...
		lpTune = tstr_createUTF8(TEXT("zerolatency"));
        if (x264_param_default_preset(&paramData, lpPreset, lpTune))
            Log(TEXT("Failed to set x264 defaults: %s/"), curPreset.Array());
		...
		paramData.i_keyint_max = fps;
		paramData.i_keyint_min = fps;

        paramData.pf_log                = get_x264_log;
        paramData.i_log_level           = X264_LOG_WARNING;

		//x264_param_apply_profile(&paramData, "main");
	  
		//* 速率控制参数   
		paramData.rc.i_bitrate = maxBitrate;//* 码率(比特率,单位Kbps)
	    paramData.rc.i_vbv_max_bitrate = paramData.rc.i_vbv_buffer_size = 1.2*maxBitrate; 
		paramData.rc.f_rate_tolerance = 0.1;
		...
		//x264_param_default_preset(&paramData,tstr_createUTF8(TEXT("superfast")),tstr_createUTF8(TEXT("zerolatency")));
    ...
    void GetHeaders(DataPacket &packet)
    {
        if(!HeaderPacket.Num())
        {
            ...
            x264_nal_t *nalOut;
            int nalNum;
			char nal_code[4] = {0x0,0,0,1};
            x264_encoder_headers(x264, &nalOut, &nalNum);
			char *tmp = sps_and_pps_buf;
            for(int i=0; i<nalNum; i++)
            {
                x264_nal_t &nal = nalOut[i];
                ...
                if(nal.i_type == NAL_SPS)
                {
					...
					memcpy(tmp,nal_code,4);
					memcpy(tmp + 4,nal.p_payload+4,nal.i_payload-4);
					sps_pps_len += nal.i_payload;
					tmp += nal.i_payload;
                    headerOut.OutputByte(0x17);
                    headerOut.OutputByte(0);
                    headerOut.OutputByte(0);
                    ...
                    x264_nal_t &pps = nalOut[i+1]; //the PPS always comes after the SPS
					memcpy(tmp,nal_code,4);
					memcpy(tmp + 4,pps.p_payload+4,pps.i_payload-4);
					sps_pps_len += pps.i_payload;
					tmp += pps.i_payload;
                    headerOut.OutputByte(1);
                    headerOut.OutputWord(htons(pps.i_payload-4));
                    headerOut.Serialize(pps.p_payload+4, pps.i_payload-4);
	...
	char* Get_SpsAndPps(char* pdata,int& datalen)
	{
		memcpy(pdata,sps_and_pps_buf,sps_pps_len);
		datalen = sps_pps_len;
