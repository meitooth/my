            ...
            if(firstFrameTime < pendingAudioFrames[0].timestamp)
            {
                ...
                if(audioTimestamp == 0 || audioTimestamp > lastAudioTimestamp)
                {
                    ...
                    if(audioData.Num())
                    {
						...
						OSDebugOut(TEXT("audio time %d size %d \r\n"),audioTimestamp,audioData.Num());
                        if(network)
                            network->SendPacket(audioData.Array(), audioData.Num(), audioTimestamp, PacketType_Audio);
                        if(fileStream&&!network)
...
bool OBS::ProcessFrame(FrameProcessInfo &frameInfo)
{
    ...
    if(bProcessedFrame)
    {
		...
		SendRtpPsToServer(spspps,spslen, (uint8_t*)videoPackets.Array()->lpPacket,videoPackets.Array()->size,bufferedTimes[0]);
...
void OBS::EncodeLoop()
{
    ...
    while(!bShutdownEncodeThread || (bufferedFrames && !bTestStream)) {
        if (!SleepToNS(sleepTargetTime += (frameTimeNS/4)))
            no_sleep_counter++;
        else
            no_sleep_counter = 0;
        ...
        if (!SleepToNS(sleepTargetTime += (frameTimeNS*3/4)))
            no_sleep_counter++;
        else
            no_sleep_counter = 0;
